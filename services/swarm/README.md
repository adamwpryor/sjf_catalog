...
The Python agent layer ported from legacy-client (`CatalogProductionWizard` + `/api/catalog/*` +
...
- `vendor/` — faithful legacy-client snapshots, pinned in `UPSTREAM.lock`. **Never edit these**; a re-sync
...
> The `delta-processor`, `curriculum-auditor`, and `diagnostics-analyst` routes were inherited
> placeholder stubs that returned canned success text without doing any work. They were **removed**
> in Phase 7 rather than left to mislead an operator exploring the API. The real remediation lives
> in the Next.js `/api/catalog/remediate` route.
