...
def _anthropic_client():
    """Builds the generation client for every agent in this service.

    The name is historical: the agents below are written against the Anthropic
    message-shaped API, but this deployment has no Anthropic key path. The client
    returned here is the Vertex shim in ``overrides/vertex.py``, which presents that
    same surface while calling Vertex Gemini with Application Default Credentials.

    Returns:
        VertexShimClient: A keyless client exposing ``.messages.create(...)``.
    """
    return override_anthropic_client()
...
app = FastAPI(title="SJFU Catalog Swarm API (Vertex AI)", version="1.0")
...
class _BearerAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not _SWARM_TOKEN:
            logger.warning(
                "Refusing %s %s: SWARM_API_TOKEN is not configured, so no request can be "
                "authenticated. Set the secret to enable the API.",
                request.method,
                request.url.path,
            )
            return JSONResponse({"detail": "Unauthorized: SWARM_API_TOKEN is not set"}, status_code=401)
...
if not _SWARM_TOKEN:
    logger.warning(
        "SWARM_API_TOKEN is unset — every authenticated route will refuse with 401 until it is "
        "configured. Only /health and the docs endpoints will respond."
    )
...
def health_check() -> dict[str, str]:
    ...
    Example:
        ...
        'ok'
    """
    return {"status": "ok", "service": "sjf-catalog-swarm-api"}

@app.post("/api/agent/extract-minutes")
async def extract_minutes_agent(file: UploadFile = File(...)) -> dict[str, Any]:  # noqa: B008
...
def render_pdf(req: RenderPdfRequest) -> Response:
    ...
    try:
        from weasyprint import (
            HTML,  # imported lazily; only present in the Cloud Run image
        )
...
You are the Catalog Correction Assistant for {institution}. A registrar is viewing \
...
DATA actions (the catalog content or structure must actually change):
   - "promote": make a nested subsection its OWN TOP-LEVEL section (a heading-LEVEL / hierarchy \
     change). match = the exact subsection heading (e.g. "General Information"). \
   ...
   - "merge": absorb a whole section into another heading — i.e. eliminate a leftover/duplicate \
     ...
     Information", a leftover "College Calendar" header remains below it covering general \
     content -> merge match:"2025-2026 College Calendar" value:"General Information".
...
# The prompt carries a single ``{institution}`` placeholder rather than a hardcoded name, so this
# service can be stood up for another institution by setting one environment variable. Formatting
# once at import time keeps the per-request path unchanged.
INSTITUTION_NAME = os.environ.get("INSTITUTION_LEGAL_NAME", "St. John Fisher University")
CORRECTION_SYSTEM_PROMPT = CORRECTION_SYSTEM_PROMPT.replace("{institution}", INSTITUTION_NAME)
