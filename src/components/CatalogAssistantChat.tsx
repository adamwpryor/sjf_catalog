...
function formatMarkdownToHTML(markdown: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
      ...
      htmlOutput.push(`<h1 style="color: #993333; font-family: Georgia, serif; font-size: 15pt; margin-top: 20px; margin-bottom: 8px; border-bottom: 1px solid #E2E8F0; padding-bottom: 4px;">${text}</h1>`);
    } else if (line.startsWith('## ')) {
      const text = line.substring(3);
      htmlOutput.push(`<h2 style="color: #993333; font-family: Georgia, serif; font-size: 13pt; margin-top: 16px; margin-bottom: 6px;">${text}</h2>`);
    } else if (line.startsWith('### ')) {
      const text = line.substring(4);
      htmlOutput.push(`<h3 style="color: #993333; font-family: Georgia, serif; font-size: 11pt; margin-top: 12px; margin-bottom: 4px;">${text}</h3>`);
    }
    // Handle List Items
    else if (isUlItem) {
      const text = line.substring(2);
...
function TerminalLogsDrawer({ logs }: { logs: { query: string; result: string; timestamp: string }[] }) {
  ...
  return (
    <div className="mt-3 w-full border border-[#FFCC33]/15 rounded-xl overflow-hidden bg-[#3d1010] transition-all">
      <button 
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-4 py-2.5 text-[10px] font-bold font-mono tracking-wider uppercase text-[#FFCC33] hover:bg-white/5 transition-all cursor-pointer border-none bg-transparent"
      >
      ...
      {isOpen && (
        <div className="p-3.5 bg-black/45 border-t border-[#FFCC33]/10 font-mono text-[9px] text-[#A6C0CA] space-y-2 max-h-60 overflow-y-auto leading-normal">
          {logs.map((log, idx) => (
            <div key={idx} className="border-b border-white/5 pb-2 last:border-0 last:pb-0">
              <div className="flex justify-between items-center text-[#FFCC33]/40 mb-1 text-[8px]">
              ...
              <pre className="bg-[#521a1a]/80 p-2 rounded border border-white/5 overflow-x-auto max-w-full text-slate-300 select-all whitespace-pre-wrap font-mono text-[8px]">
                {log.result}
              </pre>
            </div>
          ))}
...
export default function CatalogAssistantChat({ catalogId }: CatalogAssistantChatProps) {
      ...
      messages.forEach((msg) => {
        ...
        outputLines.push(`
          <div style="margin-bottom: 25px; page-break-inside: avoid; border-bottom: 1px solid #F1F5F9; padding-bottom: 15px;">
            <p style="font-weight: bold; color: ${msg.sender === 'user' ? '#993333' : '#0F172A'}; font-size: 11pt; margin-bottom: 6px; font-family: 'Arial', sans-serif; text-transform: uppercase; letter-spacing: 0.5px;">
              ${roleLabel}
            </p>
      ...
      const htmlContent = `
        ...
        <head>
          ...
          <style>
            ...
            h2 { color: #993333; font-family: 'Georgia', serif; font-size: 18pt; margin-bottom: 5px; }
        ...
        <body>
          ...
          <hr style="border:0;border-top:2px solid #993333;margin-bottom:25px;">
          ${outputLines.join('')}
        </body>
        </html>
      `;
      ...
      messages.forEach((msg) => {
        ...
        outputLines.push(`
          <div style="margin-bottom: 25px; page-break-inside: avoid; border-bottom: 1px solid #E2E8F0; padding-bottom: 15px;">
            <div style="font-weight: bold; color: ${msg.sender === 'user' ? '#993333' : '#0F172A'}; font-size: 10pt; text-transform: uppercase; margin-bottom: 6px; font-family: sans-serif; letter-spacing: 0.5px;">
              ${roleLabel}
            </div>
      ...
      printWindow.document.write(`
        ...
        <head>
          ...
          <style>
            ...
            h1 { font-family: Georgia, serif; color: #993333; font-size: 20pt; margin-bottom: 5px; }
            .header { border-bottom: 3px solid #993333; padding-bottom: 15px; margin-bottom: 30px; }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] animate-in fade-in duration-300 font-sans">
      ...
      <div className="bg-[#521a1a] p-4 rounded-xl border border-[#FFCC33]/10 mb-4 shrink-0 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between text-left">
        <div>
        ...
        <div className="relative group self-stretch md:self-auto flex">
          ...
          <div className="absolute right-0 pt-2 w-full md:w-40 hidden group-hover:block z-20">
            <div className="bg-[#521a1a] rounded-lg shadow-xl border border-[#FFCC33]/15 overflow-hidden">
              <button 
                type="button" 
      ...
      <div className={`p-4 rounded-xl border mb-4 shrink-0 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between text-left transition-all ${
        mode === 'RAG' 
          ? 'bg-[#521a1a]/50 border-[#FFCC33]/10' 
          : 'bg-[#062419]/35 border-emerald-500/20'
      }`}>
        
        {/* Mode Radio Toggles */}
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-bold text-[#FFCC33] uppercase tracking-wider font-mono">Assistant Mode:</span>
          ...
          {mode === 'RAG' ? (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-[8px] font-extrabold bg-[#993333]/25 text-[#FFCC33] border border-[#993333]/45 font-mono uppercase tracking-wider animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <span className="text-[10px] font-bold text-[#FFCC33] uppercase tracking-wider font-mono">Cognitive Model:</span>
          <select
            value={selectedModel}
      ...
      <div className={`flex-1 overflow-y-auto p-6 rounded-2xl border space-y-4 mb-4 relative transition-all duration-300 ${
        mode === 'RAG' 
          ? 'bg-[#521a1a] border-[#FFCC33]/10 shadow-inner' 
          : 'bg-[#3d1010] border-emerald-500/10 shadow-lg shadow-emerald-500/5'
      }`}>
        
        {/* Floating Active Banner Indicator */}
        <div className="absolute top-4 right-6 z-10 hidden md:block">
          {mode === 'RAG' ? (
            <div className="bg-[#521a1a] px-3.5 py-1.5 rounded-xl border border-[#FFCC33]/10 text-[9px] font-mono font-bold text-slate-400 tracking-wider shadow-lg flex items-center gap-2">
          ...
          return (
            ...
            >
              ...
              <div className={`p-4 rounded-2xl border text-xs leading-relaxed font-medium text-left ${
                isUser 
                  ? 'bg-[#993333] text-white border-[#993333]/50 rounded-br-none shadow-md shadow-[#993333]/10'
                ...
                <div className="markdown-content">
                  <ReactMarkdown 
                    ...
                    components={{
                      h1: ({node, ...props}) => <h1 className="text-sm font-extrabold mt-4 mb-2 text-white border-b border-[#FFCC33]/10 pb-1" {...props} />,
              ...
              {!isUser && (
                <div className="mt-2 flex flex-col gap-1.5 text-[9px] font-mono text-slate-500 text-left w-full pl-2">
                  <div className="flex items-center gap-1.5 font-bold uppercase tracking-widest text-[#FFCC33]/50">
                    <span className={`w-1.5 h-1.5 rounded-full ${msg.vectorSearch !== undefined ? (msg.vectorSearch ? 'bg-emerald-400' : 'bg-[#993333]') : 'bg-[#FFCC33]/50'}`}></span>
                    <span>
                      ...
                      {msg.sources.map((src, i) => {
                        ...
                        return (
                          <button
                            ...
                            className="px-2 py-0.5 rounded bg-white/5 border border-white/10 hover:bg-white/10 hover:border-[#FFCC33]/35 text-slate-400 hover:text-white transition-all truncate max-w-[180px] cursor-pointer flex items-center gap-1 font-mono text-[9px] text-left"
        ...
        {loading && (
          <div className="mr-auto items-start flex flex-col max-w-[80%] animate-pulse">
            <div className="p-4 rounded-2xl border bg-white/5 border-white/5 rounded-bl-none text-xs text-slate-400 flex items-center gap-2">
              <svg className="animate-spin h-3.5 w-3.5 text-[#FFCC33]" fill="none" viewBox="0 0 24 24">
      ...
      <form onSubmit={handleSend} className="flex gap-3 shrink-0 items-end">
        <textarea
          ...
          className={`flex-1 bg-[#521a1a] border rounded-xl px-5 py-3 text-xs text-white placeholder-slate-500 outline-none transition-all disabled:opacity-50 resize-none overflow-y-auto min-h-[96px] max-h-[150px] ${
            mode === 'RAG' 
              ? 'border-[#FFCC33]/15 focus:border-[#993333] focus:ring-1 focus:ring-[#993333]' 
        ...
        <button
          ...
          className={`text-white rounded-xl px-6 font-semibold text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 border-none shrink-0 h-[46px] self-end ${
            mode === 'RAG'
              ? 'bg-[#993333] hover:bg-[#7a2929] active:scale-[0.98] shadow-lg hover:shadow-[#993333]/25'
          ...
          <div className="bg-[#521a1a] border border-[#FFCC33]/30 rounded-xl w-full max-w-2xl flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-white/10 bg-black/50 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2 text-[#FFCC33]">
                <svg className="w-5 h-5 text-[#FFCC33]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            ...
            <div className="px-6 py-4 border-t border-white/10 bg-black/50 flex justify-end gap-3 shrink-0">
              <button onClick={() => setSelectedSource(null)} className="px-5 py-2 bg-[#993333] hover:bg-[#7a2929] text-white rounded-lg text-xs font-bold transition-all cursor-pointer">
                Close
              </button>
            </div>
          </div>
