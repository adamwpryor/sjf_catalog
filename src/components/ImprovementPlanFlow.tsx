...
const CRITERION_COLORS = ['#993333', '#FFCC33', '#d9a441', '#5b8c5a', '#7b6cb0', '#3b82a0', '#c0673a'];
...
function InitiativeNode({ data }: NodeProps) {
  ...
  return (
    <div
      className={`w-[230px] rounded-xl border bg-[#521a1a] shadow-lg transition-all ${
        selected ? 'border-[#FFCC33] ring-2 ring-[#FFCC33]/40' : 'border-white/10 hover:border-white/25'
      } ${suggested ? 'border-dashed' : ''}`}
    >
      <Handle type="target" position={Position.Left} className="!bg-[#FFCC33] !w-2 !h-2" />
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between gap-1">
          ...
          {suggested && <span className="text-[9px] font-mono text-[#FFCC33]/70">suggested</span>}
        </div>
      </div>
      <Handle type="source" position={Position.Right} className="!bg-[#FFCC33] !w-2 !h-2" />
    </div>
  );
}
...
function FlowInner(props: ImprovementPlanFlowProps) {
    ...
    for (const p of plans) {
      for (const dep of p.depends_on || []) {
        ...
        nextEdges.push({
          ...
          target: p.id,
          animated: false,
          style: { stroke: '#FFCC33', strokeWidth: 1.5 },
          markerEnd: { type: MarkerType.ArrowClosed, color: '#FFCC33' },
        });
      }
    }
    setNodes(nextNodes);
  ...
  return (
    <div className="relative flex-1 rounded-xl border border-[#FFCC33]/10 bg-[#3d1010] overflow-hidden">
      {/* Toolbar */}
      ...
      {plans.length === 0 ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-8 text-slate-500">
          <svg className="w-12 h-12 text-[#993333]/40 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
          <p className="text-sm font-medium text-slate-300 mb-1">No improvement plan yet</p>
          <p className="text-xs max-w-xs">Use the assistant on the left to <span className="text-[#FFCC33]">Generate a Plan</span>, or add an item manually. Suggested initiatives will appear here as a dependency map.</p>
        </div>
      ) : (
        <ReactFlow
          nodes={nodes}
          ...
          minZoom={0.2}
        >
          <Background variant={BackgroundVariant.Dots} gap={20} size={1} color="#5c1f1f" />
          <Controls className="!bg-[#521a1a] !border !border-white/10 [&>button]:!bg-[#521a1a] [&>button]:!border-white/10 [&>button]:!fill-slate-300" />
        </ReactFlow>
      )}
...
function DetailDrawer({ item, currentYear, yearOptions, canEdit, onClose, onPatch, onExplain, onDelete }: DetailDrawerProps) {
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-full sm:w-[380px] bg-[#521a1a] border-l border-[#FFCC33]/20 shadow-2xl z-20 flex flex-col animate-in slide-in-from-right duration-200">
        ...
        {!editing ? (
          <>
            ...
            <div className="flex flex-wrap items-center gap-2 text-[10px] font-mono">
              ...
              <span className="px-2 py-0.5 rounded bg-[#FFCC33]/10 text-[#FFCC33] border border-[#FFCC33]/20">{PLAN_STATE_LABEL[item.plan_state]}</span>
            </div>
            {item.description && <p className="text-sm text-slate-300 leading-relaxed">{item.description}</p>}
            {item.rationale && (
              <div className="border-l-2 border-[#993333] bg-[#993333]/5 p-3 rounded-r-lg">
                <div className="text-[9px] uppercase tracking-widest text-[#993333] font-mono font-bold mb-1">Why this maps to {item.criterion_code || 'the criterion'}</div>
            ...
            <div>
              <button
                onClick={runExplain}
                disabled={explaining}
                className="w-full text-xs font-semibold px-3 py-2 rounded-lg bg-[#993333] hover:bg-[#a32a3c] disabled:opacity-50 text-white transition-colors flex items-center justify-center gap-2"
              >
                {explaining ? (
              ...
              {explain && (
                <div className="mt-3 text-sm text-slate-300 bg-black/30 border border-white/5 rounded-lg p-3">
                  <ReactMarkdown
                    ...
                    components={{
                      h2: ({ node, ...props }) => <h2 className="text-[11px] font-bold uppercase tracking-wider text-[#FFCC33] mt-3 mb-1 first:mt-0" {...props} />,
          ...
          <>
            <div>
              ...
              <input value={draft.title} onChange={e => setDraft({ ...draft, title: e.target.value })}
                className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#993333]" />
            </div>
            <div>
              ...
              <textarea value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })}
                className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#993333] h-24" />
            </div>
        ...
        {!editing ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={saveAmendment} disabled={busy}
              className="text-[11px] font-bold px-3 py-2 rounded-lg bg-[#993333] hover:bg-[#a32a3c] disabled:opacity-50 text-white transition-colors">
              Save amendment
            </button>
